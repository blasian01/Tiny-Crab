/**
 * Tiny Crab hints protocol.
 *
 * CLIs and SDKs running under Tiny Crab can emit a self-closing
 * `<tiny-crab-hint />` tag to stderr (merged into stdout by the shell
 ...
 *
 * See docs/tiny-crab-hints.md for the vendor-facing spec.
 */
 ...
 */
const HINT_TAG_RE = /^[ \t]*<tiny-crab-hint\s+([^>]*?)\s*\/>[ \t]*$/gm
...
export function extractClaudeCodeHints(
  ...
  if (!output.includes('<tiny-crab-hint')) {
