...
export const CCR_TERMS_URL = 'https://code.claude.com/docs/en/tiny-crab-on-the-web';
...
let _rawPrompt: string | {
  default?: string;
} = 'Draft a concrete, step-by-step implementation plan, then wait for approval before coding.';
/* eslint-disable @typescript-eslint/no-require-imports */
try {
  _rawPrompt = require('../utils/ultraplan/prompt.txt') as string | {
    default?: string;
  };
} catch {
  logForDebugging('[ultraplan] prompt.txt missing; using fallback instructions');
}
/* eslint-enable @typescript-eslint/no-require-imports */
const DEFAULT_INSTRUCTIONS: string = (typeof _rawPrompt === 'string' ? _rawPrompt : _rawPrompt.default ?? '').trimEnd() || 'Draft a concrete, step-by-step implementation plan, then wait for approval before coding.';
    ...
    try {
      ...
      if (executionTarget === 'remote') {
        ...
        enqueuePendingNotification({
          value: [`Ultraplan approved — executing in Tiny Crab on the web. Follow along at: ${url}`, '', 'Results will land as a pull request when the remote session finishes. There is nothing to do here.'].join('\n'),
...
export async function launchUltraplan(opts: {
  ...
  if (!blurb && !seedPlan) {
    ...
    'Usage: /ultraplan \\<prompt\\>, or include "ultraplan" anywhere', 'in your prompt', '', 'Advanced multi-agent plan mode with our most powerful model', '(Opus). Runs in Tiny Crab on the web. When the plan is ready,', 'you can execute it in the web session or send it back here.', 'Terminal stays free while the remote plans.', 'Requires /login.', '', `Terms: ${CCR_TERMS_URL}`].join('\n');
  }
...
export default {
  type: 'local-jsx',
  name: 'ultraplan',
  description: `~10–30 min · Tiny Crab on the web drafts an advanced plan you can edit and approve. See ${CCR_TERMS_URL}`,
  argumentHint: '<prompt>',
