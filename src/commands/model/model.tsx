...
export const call: LocalJSXCommandCall = async (onDone, _context, args) => {
  args = args?.trim() || '';
  const isLocalMode = isLocalModelMode();
  ...
  if (COMMON_HELP_ARGS.includes(args)) {
    onDone(isLocalMode ? 'Run /model <local-model-id> to switch models. Tiny Crab local mode does not use the built-in Claude model menu.' : 'Run /model to open the model selection menu, or /model [modelName] to set the model.', {
      display: 'system'
    });
    return;
  }
  ...
  if (isLocalMode) {
    onDone('Use /model <local-model-id> to switch models. Restart tiny-crab to use the startup picker for installed local models.', {
      display: 'system'
    });
    return;
  }
