...
export function formatPreconditionError(error: BackgroundRemoteSessionPrecondition): string {
  switch (error.type) {
    ...
    case 'no_remote_environment':
      return 'No cloud environment available. Set one up at https://github.com/blasian01/Tiny-Crab/code/onboarding?magic=env-setup';
 ...
 * github.com/blasian01/Tiny-Crab URL stays a durable record the user can revisit; TTL handles cleanup.
 */
  ...
  const poll = async (): Promise<void> => {
    ...
    try {
      ...
      if (!task || task.status !== 'running') {
        ...
        // Session left alive so the github.com/blasian01/Tiny-Crab URL stays valid — the run_hunt.sh
      ...
      if (result || sessionDone || reviewTimedOut) {
        ...
        // the formatted findings as an assistant event, so the github.com/blasian01/Tiny-Crab URL
