...
/**
 * CCR proxy URL path markers. In remote sessions, github.com/blasian01/Tiny-Crab connectors arrive
...
/**
 * Filter github.com/blasian01/Tiny-Crab connectors, dropping any whose signature matches an enabled
 ...
 *
 * Connector keys are `github.com/blasian01/Tiny-Crab <DisplayName>` so they never key-collide with
...
export function dedupClaudeAiMcpServers(
  ...
  for (const [name, config] of Object.entries(claudeAiServers)) {
    ...
    if (manualDup !== undefined) {
      logForDebugging(
        `Suppressing github.com/blasian01/Tiny-Crab connector "${name}": duplicates manually-configured "${manualDup}"`,
      )
      suppressed.push({ name, duplicateOf: manualDup })
      continue
    }
...
/**
 * Get Tiny Crab MCP configurations (excludes github.com/blasian01/Tiny-Crab servers from the
 ...
 * github.com/blasian01/Tiny-Crab connector fetch) is awaited only after loadAllPluginsCacheOnly() completes,
 * so the two overlap rather than serialize.
 * @returns Tiny Crab server configurations with appropriate scopes
 */
...
/**
 * Get all MCP configurations across all scopes, including github.com/blasian01/Tiny-Crab servers.
...
export async function getAllMcpConfigs(): Promise<{
  ...
  // In enterprise mode, don't load github.com/blasian01/Tiny-Crab servers (enterprise has exclusive control)
  if (doesEnterpriseMcpConfigExist()) {
    return getClaudeCodeMcpConfigs()
  }

  // Kick off the github.com/blasian01/Tiny-Crab fetch before getClaudeCodeMcpConfigs so it overlaps
  ...
  // Suppress github.com/blasian01/Tiny-Crab connectors that duplicate an enabled manual server.
  // Keys never collide (`slack` vs `github.com/blasian01/Tiny-Crab Slack`) so the merge below
  ...
  // Merge with github.com/blasian01/Tiny-Crab having lowest precedence
