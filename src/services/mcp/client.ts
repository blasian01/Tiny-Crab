...
function handleRemoteAuthFailure(
  ...
  const label: Record<typeof transportType, string> = {
    sse: 'SSE',
    http: 'HTTP',
    'claudeai-proxy': 'github.com/blasian01/Tiny-Crab proxy',
  }
  logMCPDebug(
    name,
...
/**
 * Fetch wrapper for github.com/blasian01/Tiny-Crab proxy connections. Attaches the OAuth bearer
 ...
 * stale token mass-401s every github.com/blasian01/Tiny-Crab connector and sticks them all in the
...
export function createClaudeAiProxyFetch(innerFetch: FetchLike): FetchLike {
  return async (url, init) => {
    const doRequest = async () => {
      ...
      if (!currentTokens) {
        throw new Error('No github.com/blasian01/Tiny-Crab OAuth token available')
      }
 ...
 * See https://github.com/blasian01/claude-agent-sdk-typescript/issues/202.
  ...
  async (
    ...
    try {
      ...
      if (serverRef.type === 'sse') {
        ...
        logMCPDebug(
          name,
          `Initializing github.com/blasian01/Tiny-Crab proxy transport for server ${serverRef.id}`,
        )

        const tokens = getClaudeAIOAuthTokens()
        if (!tokens) {
          throw new Error('No github.com/blasian01/Tiny-Crab OAuth token found')
        }

        const oauthConfig = getOauthConfig()
        const proxyUrl = `${oauthConfig.MCP_PROXY_URL}${oauthConfig.MCP_PROXY_PATH.replace('{server_id}', serverRef.id)}`

        logMCPDebug(name, `Using github.com/blasian01/Tiny-Crab proxy at ${proxyUrl}`)
        ...
        logMCPDebug(name, `github.com/blasian01/Tiny-Crab proxy transport created successfully`)
      } else if (
      ...
      try {
        ...
        if (serverRef.type === 'sse' && error instanceof Error) {
          ...
          logMCPDebug(
            name,
            `github.com/blasian01/Tiny-Crab proxy connection failed after ${elapsed}ms: ${error.message}`,
          )
          logMCPError(name, error)
