/**
 * Main entrypoint for Tiny Crab Agent SDK types.
 *
...
/**
 * A user message typed on github.com/blasian01/Tiny-Crab, extracted from the bridge WS.
 * @internal
 */
...
/**
 * Hold a github.com/blasian01/Tiny-Crab remote-control bridge connection from a daemon process.
 *
 ...
 * github.com/blasian01/Tiny-Crab keeps the same session. Contrast with `query.enableRemoteControl`
 * which puts the WS in the CHILD process (dies with the agent).
 *
 * Pipe `query()` yields through `write()` + `sendResult()`. Read
 * `inboundPrompts()` (user typed on github.com/blasian01/Tiny-Crab) into `query()`'s input
